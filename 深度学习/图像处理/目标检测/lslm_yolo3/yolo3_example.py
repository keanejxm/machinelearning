...
"""
:author: keane
:file  yolo3_example.py
:time  2024/1/25 17:03
:desc  
"""
# -*- coding: UTF-8 -*-
"""
...
# 定义一个字典，将所有全局变量存入字典中
train_params = {
    "data_dir": "E:\keane_data\data_test\lslm",  # 数据目录
    "train_list": "train.txt",  # 训练集文件
    "eval_list": "eval.txt",  # 评估数据集
    "class_dim": -1,  # 类别数量(初始化时候再设置)
    "label_dict": {},  # 标签字典
    "num_dict": {},  # 数字-名称对应字典
    "image_count": -1,  # 样本数量(初始化时候再设置)
    ...
    "early_stop": {  # 提前停止策略(训练轮次太多会导致过拟合)
        "sample_frequency": 50,
        "successive_limit": 3,
        "min_loss": 2.5,
        "min_curr_map": 0.84
